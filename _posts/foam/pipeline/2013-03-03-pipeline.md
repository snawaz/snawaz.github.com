...
Pipeline is an incubation effort to mimic [LINQ-to-Objects][linq2obj] to a great extent, with few differences such as Pipeline supports composition in a way that LINQ doesn't, as a result of which you can *reuse* a query written in Pipeline for as many data source as you want. In LINQ, that is not possible.
...
As we can see, there is no *participants* collection which the *upipeline* is bounded to; *upipeline* is an unbounded pipeline. Once we've unbounded pipeline, we can reuse it as:
...
//we can do these too!
auto nameSelector = transform([](person p) { return p.name; }); //this is an unbounded pipeline!
auto teenFilter = filter([](person p) { return p.age >= 13 && p.age <=19 }); //unbounded pipeline!

f(nameSelector);               //select name of ALL persons (since no filter is applied!)
f(teenFilter | nameSelector ); //select name of teenagers only (same as upipeline!)
{% endhighlight %}
